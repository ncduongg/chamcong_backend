...
module.exports.getUserbyId = (req, res, next) => {
    ...
    User.find({ idUser: idUser }).then(user => {
...
module.exports.filterUser = async (req, res, next) => {
    ...
    const dateStart = moment(req.query.dateStart).subtract(7,'hour')
    const dateEnd = moment(req.query.dateEnd).subtract(7,'hour')
    ...
    if(idUser === "null"){
        ...
        }).where("local").in(idLocal).sort({ idUser:'asc',date: 'asc' }).then(user => {
            if (user.length > 1) {
    ...
    if(idUser !== "null"){
        ...
        }).where("local").in(idLocal).sort({ date: 'asc' }).then(user => {
            if (user.length > 1) {
                res.status(200).json(user)
            }
            else res.json({ err: "Id nhân viên không đúng" })
